...
void sendImageToServer(uint8_t *data, size_t len)
{
    ...
    // If server accepted the card, wait longer before next scan
    if (httpCode == 200 && response.indexOf("\"status\":\"success\"") >= 0 && response.indexOf("\"error\"") < 0 // no error field
        && response.indexOf("type_line") >= 0)
    { // scryfall data present
        Serial.println("[CAM] Card identified, waiting before next scan...");
        delay(4000);
    }
    else
    {
        Serial.println("[CAM] Card not identified, retrying soon...");
        delay(1000);
    }
    http.end();
}
